    ...
    return (
        <main className="mx-auto max-w-7xl md:p-10">
            <div className="mt-8 flex flex-col items-start justify-between gap-4 border-b border-gray-200 pb-5 sm:flex-row sm:items-center sm:gap-0">
                <h1 className="mb-3 fotn-bold text-5xl text-gray-900">My Files</h1>
                <UploadButton />
            </div>
            ...
            {files && files?.length !== 0 ? (
                <ul className="mt-8 grid grid-cols-1 gap-6 divide-y divide-zinc-200 md:grid-cols-2 lg:grid-cols-3">
                    {files.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((file) => (
                        <li key={file.id} className="col-span-1 divide-y divide-gray-200 roudned-lg bg-white shadow transition hover:shadow-lg">
                ...
                <div className="mt-16 flex flex-col items-center gap-2">
                    ...
                    <h3 className="font-semibold text-xl">Pretty emtpy around here</h3>
